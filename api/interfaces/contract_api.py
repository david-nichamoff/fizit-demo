...
class ContractAPI:
    ...
    def get_contract_dict(self, contract_idx, api_key, parties):
        ...
        try:
            ...
            contract_dict["min_threshold_amt"] = f'{Decimal(contract[11]) / 100:.2f}'
            contract_dict["max_threshold_amt"] = f'{Decimal(contract[12]) / 100:.2f}'
    ...
    def build_contract(self, contract_dict, contract_idx):
        ...
        contract.append(int(Decimal(contract_dict["min_threshold_amt"]) * 100))
        contract.append(int(Decimal(contract_dict["max_threshold_amt"]) * 100))
    ...
    def validate_contract_data(self, contract_dict):
        ...
        if not isinstance(contract_dict["min_threshold_amt"], str) or not ContractAPI.validate_amount(contract_dict["min_threshold_amt"], allow_negative=True):
            raise ValueError(f"Invalid value for 'min_threshold_amt': '{contract_dict['min_threshold_amt']}.' Must be a string in the form Y.XX where Y can be any number (including negative).")
        ...
        if Decimal(contract_dict["min_threshold_amt"]) > Decimal(contract_dict["max_threshold_amt"]):
            raise ValueError(f"'min_threshold_amt' ({contract_dict['min_threshold_amt']}) must be less than or equal to 'max_threshold_amt' ({contract_dict['max_threshold_amt']}).")
