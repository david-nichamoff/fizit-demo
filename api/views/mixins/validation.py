...
class ValidationMixin:
    ...
    def _validate_contract_type(self, contract_type, domain_manager):
        valid_contract_types = domain_manager.get_contract_types()
    ...
    def _validate_contract_idx(self, contract_idx, contract_type, contract_api, delay=5, retries=3):
        ...
        for attempt in range(retries):
            ...
            if attempt == retries - 1:
                raise ValidationError(f"Validation failed for contract index {contract_type}:{contract_idx}")
    ...
    def _validate_contract(self, contract):
        ...
        if contract["funding_instr"]["bank"] not in self.context.domain_manager.get_banks():
        ...
        for instr in ["funding_instr", "deposit_instr"]:
            if instr in contract:
                if contract[instr]["bank"] not in self.context.domain_manager.get_banks():
