...
from api.operations import (
    ...
    ArtifactOperations, CsrfOperations
)

from api.utilities.bootstrap import build_app_context
from api.utilities.logging import log_info

class PurchaseEncryptionTest(TestCase):
    def setUp(self):
        self.context = build_app_context()
        self.logger = logging.getLogger(__name__)

        # Load fixture data
        fixture_path = os.path.join(os.path.dirname(__file__), 'fixtures/purchase_fiat.json')
        ...
        self.fizit_headers = {**self.common_headers, "Authorization": f"Api-Key {self.MASTER_KEY}"}
        self.affiliate_headers = {**self.common_headers, "Authorization": f"Api-Key {self.affiliate_key}"}

        # Initialize CSRF token and operations with MASTER_KEY
    ...
    def test_purchase_contract_encryption_flow(self):
        ...
        # 3. Add transactions (no settlements for purchase)
        ...
        # 4. Retrieve and verify with MASTER_KEY (should be decrypted)
        self.verify_encryption(contract_type, contract_idx, self.fizit_headers, should_be_encrypted=False)

        # 5. Retrieve and verify with Affiliate Key (should be encrypted)
        ...
        # 7. Retrieve and verify with Affiliate Key again (should be decrypted now)
        self._initialize_operations(self.affiliate_headers)
