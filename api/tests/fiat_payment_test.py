...
class PayFiatTest(TestCase):
    ...
    def test_payments(self):
        fixtures_dir = os.path.join(os.path.dirname(__file__), 'fixtures', 'fiat_payment')
    ...
    def _run_fiat_payment_tests(self, advance_filename, deposit_filename, fixtures_dir):
        try:
            ...
            self._test_get_deposits(contract_type, contract_idx, deposit_filename)
            self._test_get_residuals(contract_type, contract_idx, contract_data['contract'])
            self._test_compare_settlement_values(contract_type, contract_idx)
    ...
    def _test_get_advances(self, contract_type, contract_idx, contract_data):
        ...
        advances = self._get_data(self.payment_ops.get_advances, contract_type, contract_idx, "advance amount")
    ...
    def _test_get_deposits(self, contract_type, contract_idx, fixture_filename):
        data = self._load_fixture(os.path.dirname(__file__), os.path.join("fixtures" , "fiat_payment", fixture_filename))
        deposits = self._get_data(
            lambda ct, idx: self.payment_ops.get_deposits(ct, idx, data["params"]["start_date"], data["params"]["end_date"]),
            contract_type, contract_idx, "deposits"
        )
        self._validate_and_add_deposits(contract_type, contract_idx, deposits, data["deposits"])

    def _test_get_residuals(self, contract_type, contract_idx, contract_data):
        expected_residual_count = contract_data["extended_data"].get("residual_count", 0)
        residuals = self._get_data(self.payment_ops.get_residuals, contract_type, contract_idx, "residual amounts")
        ...
        self._process_residuals(contract_type, contract_idx, residuals)

    def _test_compare_settlement_values(self, contract_type, contract_idx):
        log_info(self.logger, f"Validating settlement values for contract {contract_idx}...")
        settlement = self._get_data(self.settlement_ops.get_settlements, contract_type, contract_idx, "settlements")[0]
    ...
    def _get_data(self, operation, contract_type, contract_idx, data_type):
        response = operation(contract_type, contract_idx)
    ...
    def _process_residuals(self, contract_type, contract_idx, residuals):
    ...
    def _validate_and_add_deposits(self, contract_type, contract_idx, deposits, expected_deposit):
