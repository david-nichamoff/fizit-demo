...
class AdvanceTokenTest(TestCase):
    ...
    def test_token_payments(self):
        ...
        fixture_path = os.path.join(os.path.dirname(__file__), "fixtures", "advance_token.json")
    ...
    def _create_contract(self, contract_type, contract_data):
        ...
        contract_data['contract_name'] = 'Advance Token Test'
    ...
    def _process_residuals(self, contract_type, contract_idx):
        """Retrieve and process residuals."""
        residuals = self.payment_ops.get_residuals(contract_type, contract_idx)
        self.assertGreater(len(residuals), 0, f"No residuals found for contract {contract_idx}")
        ...
        for attempt in range(self.retries):
            ...
            if event:
                if event["status"] == "complete":
                    ...
                    log_warning(self.logger, f"Event {event_type} found but still pending (attempt {attempt + 1}/{self.retries})...")

            else:
                log_warning(self.logger, f"Waiting for event {event_type} (attempt {attempt + 1}/{self.retries})...")
