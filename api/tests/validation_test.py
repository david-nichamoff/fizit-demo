...
class ValidationTest(TestCase):
    ...
    def _run_integrity_test(self, directory, test_method, key, error_message):
        ...
        fixtures_dir = os.path.join(os.path.dirname(__file__), 'fixtures', 'validation', directory)
        ...
        for filename in filenames:
            ...
            try:
                ...
                if key != "contract":
                    ...
                    test_method(contract_type, contract_idx, data.get(key, []), error_message)
                else:
                    test_method(contract_type, contract_data, error_message)
    ...
    def _run_contract_integrity(self, contract_type, contract_data, error_message):
        response = self.contract_ops.post_contract(contract_type, contract_data)
        log_info(self.logger, f"Checking response {response} for contract")
        self._assert_bad_request(response, error_message)

    def _run_party_integrity(self, contract_type, contract_idx, parties_data, error_message):
    ...
    def _run_settlement_integrity(self, contract_type, contract_idx, settlements_data, error_message):
    ...
    def _run_transaction_integrity(self, contract_type, contract_idx, transactions_data, error_message):
    ...
    def _assert_bad_request(self, response, error_message):
        if "error" not in response:
            self.fail(f"{error_message} Expected 400 Bad Request but got {response}")
