...
class AdvanceLifecycleTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        contract_file = os.path.join(os.path.dirname(__file__), 'fixtures', 'advance_fiat.json')

        try:
            with open(contract_file, 'r') as file:
                cls.contract_data = json.load(file)
            log_info(cls.logger, "Advance contract test data loaded successfully.")
        except FileNotFoundError as e:
            log_error(cls.logger, f"Advance contract test data file not found: {e}")
            raise
        except json.JSONDecodeError as e:
            log_error(cls.logger, f"Error decoding advance contract JSON data: {e}")
            raise
    ...
    def test_contract_lifecycle(self):
        ...
        advance_pct = float(contract_body["advance_pct"])
        ...
        self.settlement_ops.post_settlements(self.contract_type, contract_idx, settlements)
        self._validate_event(contract_idx, "SettlementAdded")
        ...
        total_transact_amt, total_service_fee, total_advance_amt = self._validate_transactions(
            transactions_data, service_fee_pct, service_fee_amt, advance_pct
        )

        # Step 9: Retrieve settlements and validate calculations
        self._validate_settlements(contract_idx, len(transactions), total_transact_amt, total_advance_amt)
        ...
        self._delete_and_validate(contract_idx, "SettlementsDeleted", self.settlement_ops.delete_settlements)
    ...
    def _validate_event(self, contract_idx, event_type):
        ...
        retries = 5
        to_addr = self.config_manager.get_contract_address("advance")

        for attempt in range(retries):
            events = self.event_ops.get_events(contract_type="advance", contract_idx=contract_idx, to_addr=to_addr)
    ...
    def _validate_transactions(self, transactions, service_fee_pct, service_fee_amt, advance_pct):
        ...
        for txn in transactions:
            ...
            expected_service_fee = (transact_amt * service_fee_pct) + service_fee_amt
            expected_advance_amt = (transact_amt * advance_pct) - expected_service_fee

            self.assertAlmostEqual(float(txn["service_fee_amt"]), expected_service_fee, places=2)
            self.assertAlmostEqual(float(txn["advance_amt"]), expected_advance_amt, places=2)

            total_transact_amt += transact_amt
            total_service_fee += expected_service_fee
            total_advance_amt += expected_advance_amt

        return total_transact_amt, total_service_fee, total_advance_amt

    def _validate_settlements(self, contract_idx, transact_count, total_transact_amt, total_advance_amt):
        """Validate settlement calculations."""
        settlements = self.settlement_ops.get_settlements(self.contract_type, contract_idx)
        self.assertEqual(settlements[0]["transact_count"], transact_count)
        self.assertAlmostEqual(float(settlements[0]["advance_amt"]), total_advance_amt, places=2)
        self.assertAlmostEqual(float(settlements[0]["settle_exp_amt"]), total_transact_amt, places=2)
    ...
    def _validate_empty(self, contract_idx):
        ...
        self.assertEqual(len(self.settlement_ops.get_settlements(self.contract_type, contract_idx)), 0)
