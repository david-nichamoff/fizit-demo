...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        def create_event_filter():
            try:
                # Create the filter with 'fromBlock', 'address' and the event topics
                event_signature_hash = self.w3.keccak(text="ContractEvent(uint256,string,string)").hex()  # Replace with your event signature
